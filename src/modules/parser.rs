...
impl Ast {
    ...
    pub fn build_imports(&mut self, cache: &mut BuildCache) {
        for ((_path, import), v) in self.imports.iter_mut() {
            ...
            for import in parse_import_path(import) {
                ...
                if let Some(function) = ast.context.functions.clone().get(import) {
                    ...
                    ast.get_st_deps(
                        &DataType::StructType(dt.name.clone()),
                        &mut rec_imports,
                        &mut impls,
                        cache,
                    );
    ...
    pub fn get_enum_dependencies(
            ...
            if let Some(ref dt) = variant.r#type {
                match dt.datatype {
                    DataType::StructType(ref name) => {
                        ...
                        let mut deps = self.resolve_deps_impls(name, &mut impls, cache);
                        deps.extend_from_slice(&rtn);
                        rtn = deps;
                    }
                    _ => continue,
                }
        ...
        let mut deps = self.resolve_deps_impls(&dt.name, &mut impls, cache);
        deps.extend_from_slice(&rtn);
        (deps, impls)
    }
    ...
    pub fn resolve_deps_impls(
        ...
        let mut rtn = Vec::new();
        if let Some(ref mut aimpls) = self.context.impls.clone().get_mut(name) {
            println!("THERE IS A IMPL FOR {name} ON CONTEXT");
    ...
    pub fn get_st_deps(
        ...
        match dt {
            ...
            DataType::StructType(ref name) => {
                ...
                //self.dt_deps.insert(name.into(), (vec![], vec![]));
                let (e, i) = if let Some(dt) = self.context.structs.clone().get(name) {
                    expr.push(Expr::StructDecl(dt.clone()));
                    self.get_struct_dependencies(dt, cache)
                } else {
                    let dt = self.context.enums.clone();
                    let dt = dt.get(name).expect(&format!(
                        "Enum {} isn't defined on {}. Defined types are {:?}",
                        name,
                        self.namespace,
                        self.context.structs.iter().map(|x| x.0).collect::<Vec<_>>()
                    ));
                    expr.push(Expr::Enum(dt.clone()));
                    self.get_enum_dependencies(dt, cache)
                };
                let (ref mut ee, ref mut ii) =
                    cache.dt_deps.entry(name.into()).or_insert((vec![], vec![]));
                let clone = ee.clone();
                let iclone = ii.clone();
                ee.extend(
                    e.into_iter()
                        .filter(|x| clone.iter().find(|y| x == *y).is_none()),
                );
                ii.extend(
                    i.into_iter()
                        .filter(|x| iclone.iter().find(|y| x == *y).is_none()),
                );
                expr.extend_from_slice(&ee);
                impls.extend_from_slice(&ii);
            }
            _ => (),
        };
    }

    pub fn get_struct_dependencies(
        &mut self,
        dt: &StructDecl,
        cache: &mut BuildCache,
    ) -> (Vec<Expr>, Vec<Impl>) {
        ...
        for field in dt.fields.iter() {
            self.get_st_deps(&field.datatype.datatype, &mut rtn, &mut impls, cache);
        }
        ...
        println!(
            ...
            dt.name,
            rtn.iter()
                .filter(|x| if let Expr::Function(_) = x {
                    true
                } else {
                    false
                })
                .map(|x| if let Expr::Function(f) = x {
                    f.name.name.clone()
                } else {
                    String::new()
                })
    ...
    fn get_from_value(
        ...
        match value {
            ValueEnum::Range(ref mut range) => match range {
                RangeValue::Iterable(ref mut iterable) => {
                    let mut iterable = iterable.borrow_mut();
                    self.get_from_value(&mut iterable.value, rtn, impls, cache);
                }
                RangeValue::Range(ref mut range) => {
                    let mut range = range.borrow_mut();
                    self.get_from_value(&mut range.start.value, rtn, impls, cache);
                    self.get_from_value(&mut range.end.value, rtn, impls, cache);
                }
            },
            ValueEnum::Call(ref call) => {
                if let Some(function) = self.context.functions.clone().get(&call.name.name) {
                    let (deps, new_impls) = self.get_fn_dependencies(function, cache);
                    rtn.extend(deps);
                    impls.extend(new_impls);
                    rtn.push(Expr::Function(function.clone()));
                }
            }
            ValueEnum::Constructor(ref constructor) => {
                let constructor = constructor.borrow();
                let dt = self.context.structs.get(&constructor.name).unwrap();
                rtn.push(Expr::StructDecl(dt.clone()));
            }
            ValueEnum::FieldAccess(ref mut rc) => {
                let access = rc.borrow();
                if let FieldAccessName::Name(ref name) = access.name {
                    if name.name.chars().next().unwrap().is_uppercase() {
                        if let Some(dt) = self.context.structs.get(&name.name) {
                            rtn.push(Expr::StructDecl(dt.clone()))
                        } else if let Some(dt) = self.context.enums.get(&name.name) {
                            rtn.push(Expr::Enum(dt.clone()))
                        } else {
                            panic!(
                                "Datatype {} is not defined for {}",
            ...
            ValueEnum::Enum(ref mut val) => {
                let mut val = val.borrow_mut();
                let dt = self.context.enums.get(&val.datatype).unwrap();
                rtn.push(Expr::Enum(dt.clone()));
                if let Some(ref mut value) = val.inner {
                    self.get_from_value(&mut value.value, rtn, impls, cache);
                }
            }
            _ => (),
        };
    ...
    fn get_from_if(
        ...
        if let Some(ref mut condition) = ast_if.condition {
            let mut condition = condition.borrow_mut();
            self.get_from_value(&mut condition.value, rtn, impls, cache);
        }
        self.get_block_dependencies(&ast_if.block.borrow(), rtn, impls, cache);
        if let Some(ref mut otherwise) = ast_if.otherwise {
            let mut otherwise = otherwise.borrow_mut();
            match *otherwise {
                Otherwise::Block(ref block) => {
                    self.get_block_dependencies(block, rtn, impls, cache)
                }
                Otherwise::If(ref mut ast_if) => self.get_from_if(ast_if, rtn, impls, cache),
            }
        }
    }

    pub fn get_block_dependencies(
        &mut self,
        block: &Block,
        rtn: &mut Vec<Expr>,
        impls: &mut Vec<Impl>,
        ...
        for expr in block.expr.iter() {
            ...
            match *expr {
                    ...
                    if let Some(ref mut otherwise) = ast_if.otherwise {
                        ...
                        match *otherwise {
                            ...
                            Otherwise::If(ref mut ast_if) => {
                                self.get_from_if(ast_if, rtn, impls, cache)
                            }
                        }
                    }
                }
                Expr::For(ref mut floop) => {
                    self.get_from_value(&mut floop.range.value, rtn, impls, cache);
                    self.get_block_dependencies(&floop.block, rtn, impls, cache);
                }
                Expr::While(ref mut wloop) => {
                    self.get_from_value(&mut wloop.condition.value, rtn, impls, cache);
                    self.get_block_dependencies(&wloop.block.borrow(), rtn, impls, cache);
                }
                Expr::Call(ref call) => {
                    if let Some(function) = self.context.functions.clone().get(&call.name.name) {
                        let (deps, new_impls) = self.get_fn_dependencies(function, cache);
                ...
                Expr::Assignment(ref mut assignment) => {
                    self.get_from_value(&mut assignment.value.value, rtn, impls, cache)
                }
                Expr::Declaration(ref mut decl) => {
    ...
    pub fn get_fn_dependencies(
        ...
        let mut rtn = Vec::new();
        let mut impls = Vec::new();
        if cache.call_stack.contains(&function.name.name) {
            return (rtn, impls);
        }
        ...
        self.get_block_dependencies(&block, &mut rtn, &mut impls, cache);
        cache.call_stack.remove(&function.name.name);
        cache
            .fn_deps
            .insert(function.name.name.clone(), (rtn.clone(), impls.clone()));
        (rtn, impls)
    }
}
...
impl NoirParser {
    pub fn new() -> Self {
        let pratt_parser = PrattParser::new()
            .op(Op::infix(Rule::bt_left, Assoc::Left)
                ...
                | Op::infix(Rule::bt_and, Assoc::Left)
                | Op::infix(Rule::bt_xor, Assoc::Left))
    ...
    fn build_expression(&self, pairs: &mut Pairs<Rule>, context: &mut AstContext) -> Vec<Expr> {
        ...
        for pair in pairs {
            let expr = match pair.as_rule() {
                ...
                Rule::macro_call => {
                    Expr::MacroCall(self.build_call(&mut pair.into_inner(), context))
                }
                Rule::function => {
                ...
                Rule::assembly => {
                    Expr::Assembly(self.build_assembly(&mut pair.into_inner(), context))
                }
                Rule::variadic_block => {
    ...
    fn build_macro(&self, pairs: &mut Pairs<Rule>, context: &mut AstContext) -> Macro {
        ...
        for pair in pairs {
            match pair.as_rule() {
                Rule::attribute => {
                    macros.push(self.build_macro_attribute(&mut pair.into_inner(), context))
                }
                _ => unreachable!(),
            }
        }
        ...
        for pair in pairs {
            match pair.as_rule() {
                ...
                Rule::asm_clob => {
                    clob = pair
                        .into_inner()
                        .map(|x| x.as_str()[1..x.as_str().len() - 1].to_string())
                        .collect()
                }
                _ => unreachable!(),
            }
            ...
            for mut eval in pair.into_inner() {
                match eval.as_rule() {
                    ...
                    | Rule::unary_operation => {
                        val = self.build_single_value(&mut eval, context).value
                    }
                    Rule::name => {
                        name = Some(self.build_name(&mut eval.into_inner(), context).name)
                    }
                    _ => unreachable!(),
                }
            }
    ...
    fn build_enum(&self, pairs: &mut Pairs<Rule>, context: &mut AstContext) -> Enum {
        ...
        for pair in pairs {
            match pair.as_rule() {
                ...
                Rule::enum_variant => {
                    variants.push(self.build_enum_variant(&mut pair.into_inner(), context))
                }
                _ => unreachable!(),
            }
        }
        let ast_enum = Enum { name, variants };
        context
            .enums
            .insert(ast_enum.name.clone(), ast_enum.clone());
        ast_enum
    }
    ...
    fn build_enum_variant(&self, pairs: &mut Pairs<Rule>, context: &mut AstContext) -> EnumVariant {
        ...
        for pair in pairs {
            match pair.as_rule() {
                ...
                Rule::datatype => {
                    r#type = Some(self.build_datatype(&mut pair.into_inner(), context))
                }
                _ => unreachable!(),
            }
        }
    ...
    fn build_impl(&self, pairs: &mut Pairs<Rule>, context: &mut AstContext) -> Impl {
        ...
        for pair in pairs {
            match pair.as_rule() {
                ...
                Rule::impl_block => {
                    (generics, methods) = self.build_impl_block(&mut pair.into_inner(), context)
                }
                _ => todo!(),
            }
        }
    ...
    fn build_impl_block(
        ...
        context: &mut AstContext,
    ) -> (Vec<(String, String)>, Vec<ImplMethod>) {
        let mut generics = vec![];
        let mut methods = vec![];
        for pair in pairs {
            match pair.as_rule() {
                Rule::function => {
                    methods.push(self.build_impl_method(&mut pair.into_inner(), context))
                }
                Rule::impl_type => {
    ...
    fn build_impl_type(&self, pairs: &mut Pairs<Rule>, context: &mut AstContext) -> ImplType {
        ...
        for pair in pairs {
            match pair.as_rule() {
                Rule::name => {
                    if switched {
                        implements.push(pair.as_str().to_string())
                    } else {
        ...
        for pair in pairs {
            match pair.as_rule() {
                ...
                Rule::trait_decl => {
                    ...
                    match next.as_rule() {
                        Rule::trait_function => {
                            methods.push(self.build_trait_method(&mut next.into_inner(), context))
                        }
                        Rule::trait_generic_type => {
                            generics.push(next.into_inner().next().unwrap().as_str().into())
                        }
                        _ => unreachable!(),
                    }
                }
    ...
    fn build_namespace(&self, pairs: &mut Pairs<Rule>) -> Namespace {
        ...
        for pair in pairs {
            match pair.as_rule() {
                ...
                Rule::namespace => {
                    next.push(Box::new(self.build_namespace(&mut pair.into_inner())))
                }
                _ => unreachable!(),
            }
        }
    ...
    fn build_single_value(&self, pair: &mut Pair<Rule>, context: &mut AstContext) -> Value {
        match pair.as_rule() {
            Rule::unary_operation | Rule::binary_operation => {
                self.build_binary_op(&mut pair.clone().into_inner(), context)
            }
                ...
                Rule::enum_build => {
                    ...
                    for mut rule in eval.into_inner() {
                        match rule.as_rule() {
                            ...
                            | Rule::binary_operation
                            | Rule::bvalue => {
                                inner = Some(self.build_single_value(&mut rule, context))
                            }
                            rule => unreachable!("Found rule {:?}", rule),
                        }
                    }
                    value.value = ValueEnum::Enum(Rc::new(RefCell::new(EnumValue {
                        datatype,
                        variant,
                        inner,
                    })))
    ...
    fn build_binary_op(&self, pairs: &mut Pairs<Rule>, context: &mut AstContext) -> Value {
        fn bop_helper(lhs: Value, op: Pair<'_, Rule>, rhs: Value) -> Value {
            Value::non_heap(ValueEnum::BinaryOp(Rc::new(RefCell::new(BinaryOp {
                lhs: Rc::new(RefCell::new(lhs)),
                op: Operations::from(op),
                rhs: Rc::new(RefCell::new(rhs)),
            }))))
        }
        fn unop_helper(value: Value, op: Pair<'_, Rule>) -> Value {
            Value::non_heap(ValueEnum::UnaryOp(Rc::new(RefCell::new(UnaryOp {
                prefix: Operations::from(op),
        ...
        self.pratt_parser
            .map_primary(|mut primary| match primary.as_rule() {
                Rule::bvalue | Rule::base_value | Rule::binary_operation => {
                    self.build_single_value(&mut primary, context)
                }
                _ => unreachable!(),
            })
    ...
    fn build_range(&self, pairs: &mut Pairs<Rule>, context: &mut AstContext) -> RangeValue {
        if pairs.len() > 1 {
            ...
            RangeValue::Iterable(Rc::new(RefCell::new(match value.as_rule() {
                Rule::unary_operation | Rule::binary_operation => {
                    self.build_binary_op(&mut value.into_inner(), context)
                }
    ...
    fn build_field(&self, pairs: &mut Pairs<Rule>, context: &mut AstContext) -> Field {
        ...
        for pair in pairs {
            match pair.as_rule() {
                Rule::name_str => name = pair.as_str().into(),
                Rule::unary_operation | Rule::bvalue => {
                    value = self.build_bvalue(&mut pair.into_inner(), context)
                }
                Rule::binary_operation => {
    ...
    fn build_array(&self, pairs: &mut Pairs<Rule>, context: &mut AstContext) -> Array {
        ...
        for pair in pairs {
            match pair.as_rule() {
                Rule::array_type => {
                    datatype = self.build_array_type(&mut pair.into_inner(), context)
                }
        ...
        let datatype = Type {
            is_ref: false,
            ref_count: 0,
            datatype: DataType::Array(Rc::new(RefCell::new(datatype))),
        };
    ...
    fn build_array_type(&self, pairs: &mut Pairs<Rule>, context: &mut AstContext) -> ArrayType {
        ...
        for pair in pairs {
            match pair.as_rule() {
                Rule::bvalue | Rule::base_value | Rule::unary_op | Rule::binary_op => {
                    size = self.build_binary_op(&mut pair.into_inner(), context)
                }
            ...
            match pair.as_rule() {
                ...
                | Rule::bvalue => {
                    value.condition = Some(Rc::new(RefCell::new(
                        self.build_single_value(&mut pair, context),
                    )))
                }
                Rule::block => {
                    ...
                    for eval in pair.into_inner() {
                        match eval.as_rule() {
                            Rule::name_str => {
                                if !switch {
                                    switch = true;
                                    name = eval.as_str().into();
                                } else {
                                    var = eval.as_str().into();
                                }
                            }
    ...
    fn build_implements_type(
        ...
        pairs
            .map(|x| {
                if let Rule::name_str = x.as_rule() {
                    x.as_str().to_string()
                } else {
                    panic!()
                }
    ...
    fn build_datatype(&self, pairs: &mut Pairs<Rule>, context: &mut AstContext) -> Type {
        ...
        for pair in pairs {
            match pair.as_rule() {
                ...
                Rule::struct_type => {
                    datatype = DataType::StructType(self.build_struct_type(&pair, context))
                }
